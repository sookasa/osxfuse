...
platforms=`$build -h|grep -i 'installed macos sdks:' | cut -d: -f2 | sed "s/^[ \t]*//"`
...
# build project
export BUILD_TARGET_OPTION_CODE_SIGN_IDENTITY=$identity
export BUILD_TARGET_OPTION_PRODUCT_SIGN_IDENTITY=$identity
cmd="$build -t distribution -v 6"
echo "-I- Running $cmd"
